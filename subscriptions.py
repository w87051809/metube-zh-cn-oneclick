...
def _subscription_asset_key(url: Any, download_type: Any, format: Any, quality: Any) -> str:
    ...
    asset = f"{download_type or 'video'}:{format or 'any'}:{quality or 'best'}".lower()
    return f"{normalized_url}#{asset}"


def _subscription_asset_label(download_type: Any, format: Any) -> str:
    ...
    if dl_type == "thumbnail":
        return "封面JPG" if dl_format == "jpg" else "封面图"
    if dl_type == "audio":
        return "音频"
    return ""
...
def _name_with_asset_label(name: Any, download_type: Any, format: Any) -> str:
    ...
    if not label or label in base_name:
        return base_name
...
class SubscriptionInfo:
    ...
    def to_public_dict(self) -> dict:
        return {
            ...
            "asset_label": _subscription_asset_label(self.download_type, self.format),
            "folder": self.folder,
...
class SubscriptionManager:
    def __init__(self, config, download_queue, notifier: SubscriptionNotifier):
        ...
        self._url_index: dict[str, str] = {}  # url + download asset -> id
    ...
    def _subscription_key(self, url: Any, download_type: Any, format: Any, quality: Any) -> str:
        return _subscription_asset_key(self._normalize_url(str(url or "")), download_type, format, quality)
    ...
    def _load_all(self) -> None:
        ...
        for sub in loaded_subs:
            ...
            self._url_index[self._subscription_key(sub.url, sub.download_type, sub.format, sub.quality)] = sub.id
    ...
    async def add_subscription(
        ...
        *,
        ...
        sub_key = self._subscription_key(url, download_type, format, quality)
        ...
        async with self._lock:
            if sub_key in self._url_index or sub_key in self._pending_keys:
                return {"status": "error", "msg": "This material is already subscribed for this URL"}
            self._pending_keys.add(sub_key)
        ...
        try:
            ...
            sub = SubscriptionInfo(
                id=str(uuid.uuid4()),
                name=_name_with_asset_label(name, download_type, format),
                url=url,
                enabled=True,
            ...
            async with self._lock:
                if sub_key in self._url_index:
                    return {"status": "error", "msg": "This material is already subscribed for this URL"}
                self._subs[sub.id] = sub
                self._url_index[sub_key] = sub.id
                try:
                    self._save_locked()
                except Exception:
                    self._subs.pop(sub.id, None)
                    self._url_index.pop(sub_key, None)
                    raise
            ...
            async with self._lock:
                self._pending_keys.discard(sub_key)
    ...
    async def delete_subscriptions(self, ids: list[str]) -> dict:
        ...
        async with self._lock:
            ...
            for sid in ids:
                ...
                if sub:
                    sub_key = self._subscription_key(sub.url, sub.download_type, sub.format, sub.quality)
                    self._url_index.pop(sub_key, None)
                    removed.append(sid)
            if removed:
                try:
