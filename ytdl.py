...
_METUBE_ASSET_FRAGMENT_PREFIX = "metube_asset="


def _strip_metube_asset_fragment(url: Any) -> str:
    value = str(url or "")
    base, sep, fragment = value.partition("#")
    if not sep:
        return value
    parts = [part for part in fragment.split("&") if not part.startswith(_METUBE_ASSET_FRAGMENT_PREFIX)]
    return base + (("#" + "&".join(parts)) if parts else "")


def _download_asset_key(url: Any, download_type: Any, format: Any, quality: Any) -> str:
    source_url = _strip_metube_asset_fragment(url)
...
class DownloadInfo:
    ...
    def __setstate__(self, state):
        ...
        if not hasattr(self, "source_url"):
            self.source_url = _strip_metube_asset_fragment(getattr(self, "url", ""))
        if not hasattr(self, "queue_key"):
            self.queue_key = getattr(self, "url", self.source_url)
    ...
    def _download(self):
        ...
        if hasattr(os, 'setpgrp'):
            try:
                ...
                pass
        download_url = getattr(self.info, "source_url", self.info.url)
...
class DownloadQueue:
    ...
    async def _probe_scheduled_download(self, download: Download) -> None:
        ...
        info = download.info
        probe_url = getattr(info, "source_url", url)
